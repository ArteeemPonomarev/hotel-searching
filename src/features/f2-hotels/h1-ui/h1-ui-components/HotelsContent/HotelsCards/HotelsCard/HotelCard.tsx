...
import {getCheckInDate} from "../../../../../h2-bll/hotels-selectors";
...
export const HotelCard: React.FC<HotelCardPropsType> = (props) => {
    ...
    const followHotel = useCallback(() => {
        ...
        dispatch(hotelsActions.addFavoriteHotel(props.hotelId, checkIn, props.amountDays));
    }, [dispatch, props.hotelId, checkIn, props.amountDays]);
    ...
    return (
        <div className={style.hotel_card}>
            <div className={style.hotel_card_description}>
                <p className={style.hotel_name}>{props.hotelName}</p>
            ...
            <div className={style.hotel_card_options}>
                ...
                <div className={style.hotel_card_price}>{props.price.toFixed()} {'\u20BD'}</div>
            </div>
        </div>
    );
};
